...
/**
 * Theme Boost isf_ingles - Library
 *
 ...
 * @return string
 */
function theme_boost_isf_ingles_get_main_scss_content($theme) {
    global $CFG;
    ...
    } else if ($filename == 'isf_ingles.scss') {
        // We still load the default preset files directly from the boost theme. No sense in duplicating them.
        $scss .= file_get_contents($CFG->dirroot . '/theme/boost_isf_ingles/scss/preset/isf_ingles.scss');

    } else if ($filename == 'isf_ingles_presencial.scss') {
        // We still load the default preset files directly from the boost theme. No sense in duplicating them.
        $scss .= file_get_contents($CFG->dirroot . '/theme/boost_isf_ingles/scss/preset/isf_ingles_presencial.scss');

    } else if ($filename && ($presetfile = $fs->get_file($context->id, 'theme_boost_isf_ingles', 'preset', 0, '/', $filename))) {
        // This preset file was fetched from the file area for theme_boost_isf_ingles and not theme_boost (see the line above).
    ...
    // $pre = file_get_contents($CFG->dirroot . '/theme/boost_isf_ingles/scss/pre.scss');
    // Post CSS - this is loaded AFTER the main scss but before the extra scss from the setting.
    // $post = file_get_contents($CFG->dirroot . '/theme/boost_isf_ingles/scss/post.scss');
 ...
 * @return array
 */
function theme_boost_isf_ingles_get_pre_scss($theme) {
    global $CFG;
    // MODIFICATION START.
    ...
    // $scss .= theme_boost_isf_ingles_get_loginbackgroundimage_scss();
    // MODIFICATION END.
 ...
 * @return bool
 */
function theme_boost_isf_ingles_pluginfile($course, $cm, $context, $filearea, $args, $forcedownload, array $options = array()) {
    if ($context->contextlevel == CONTEXT_SYSTEM) {
        $theme = theme_config::load('boost_isf_ingles');
        if ($filearea === 'favicon') {
...
/**
 * If setting is updated, use this callback to clear the theme_boost_isf_ingles' own application cache.
 */
function theme_boost_isf_ingles_reset_app_cache() {
    // Get the cache from area.
    $themeboostisf_inglescache = cache::make('theme_boost_isf_ingles', 'imagearea');
    // Delete the cache for the imagearea.
    $themeboostisf_inglescache->delete('imageareadata');
...
function get_isf_ingles_commom_moodle_template_context()
{
...
function get_isf_ingles_calendario() {
    global $CFG, $COURSE;
...
function get_isf_ingles_course_content_actions()
{
    global $PAGE, $COURSE;
...
function get_isf_ingles_course_common_actions()
{
    global $PAGE, $COURSE;
    if ($PAGE->pagelayout == "course" || $PAGE->pagelayout == "incourse") {
        $extraflatnav = [];
...
function get_isf_ingles_template_context()
{
    global $PAGE;

    $templatecontext = get_isf_ingles_commom_moodle_template_context();

    if ($templatecontext['in_course_page'] || $templatecontext['within_course_page']) {
        $templatecontext['course_content_actions'] = get_isf_ingles_course_content_actions();
        $templatecontext['course_common_actions'] = get_isf_ingles_course_common_actions();
    }
    $templatecontext['nosso_calendario'] = get_isf_ingles_calendario();
